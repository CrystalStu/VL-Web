...
$("#id").blur(function (){
	...
	if (userid.length < 4 || !regtext.test(userid)) {
		$("#i-tt1").html("<span><i class='ion-span ion-android-alert'></i>Require 4-10 valid characters (English characters, numbers, underscores)</span>");
		if(userid.length == 0){
			$("#i-tt1").html("<span><i class='ion-span ion-android-alert'></i>Please enter your Minecraft ID</span>");
		}
		namecheck = false;
	} else {
		$("#id").css("color","#666");
...
$("#em").blur(function (){
	...
	if (em.length <= 6 || !emtext.test(em)) {
		$("#i-tt2").html("<span><i class='ion-span ion-android-alert'></i>Please enter a valid email address</span>");
		if(em.length == 0){
			$("#i-tt2").html("<span><i class='ion-span ion-android-alert'></i>Please enter your email here</span>");
		}
		emcheck = false;
	} else {
		$("#em").css("color","#666");
		if(email_ != null && email_ != $('#em').val()){
			$("#i-tt3").html("<span><i class='ion-span ion-android-alert'></i>Sorry, the verification code is wrong</span>");
			$("#i-tt3").show();
			emkeycheck = false;
		} else {
			checkem(); // Get提交检测
...
$("#emkey").blur(function (){
	...
	if (emkey.length <= 5 || emkey.length > 6 || !emkeytext.test(emkey)) {
		$("#i-tt3").html("<span><i class='ion-span ion-android-alert'></i>Please enter a valid verification code</span>");
		if(emkey.length == 0){
			$("#i-tt3").html("<span><i class='ion-span ion-android-alert'></i>Please enter your email verification code</span>");
		}
		$("#i-tt3").show();
		emkeycheck = false;
	} else {
...
function sendem(){
	if(emcheck){
		...
		$('#emkey').val("");
		$("#emkey").focus();
		$("#i-tt3").html("<span style='color: #63da5c;'><i class='ion-span ion-ios-checkmark'></i>The verification code was sent to your email box, if you cannot find the mail, try to find it in the trash box</span>");
		$("#i-tt3").show();
		email_ = $("#em").val(); // 记录邮箱
		emkeycheck = false;
	}
...
function settime() {
    if(countdown == 0) {
        $("#sdem").html('Send verification code again');
        ...
        $("#sdem").html("You can try to send again in (" + countdown + ")");
		$("#sdem").addClass('btn-disabled');
		$("#sdem").attr("disabled", true);
		isCD = true;
        countdown--;
...
$("#pw").blur(function (){
	...
	if (pw.length <= 5 || !pwtext.test(pw)) {
		$("#i-tt4").html("<span><i class='ion-span ion-android-alert'></i>Please enter 6-16 valid characters (English characters, numbers, symbols)</span>");
		if(pw.length == 0){
			$("#i-tt4").html("<span><i class='ion-span ion-android-alert'></i>Please enter your password</span>");
		}
		pwcheck = false;
	} else {
...
$("#apw").blur(function (){
	...
	if (pw != apw || apw.length == 0) {
		$("#i-tt5").html("<span><i class='ion-span ion-android-alert'></i>Sorry, not the same password</span>");
		if(apw.length == 0){
			$("#i-tt5").html("<span><i class='ion-span ion-android-alert'></i>Please enter your password again to confirm</span>");
		}
		apwcheck = false;
	} else {
...
$("#fkey").blur(function (){
	...
	if (fkey.length < 4 || !fkeytext.test(fkey)) {
		if(fkey.length == 0){
			$("#i-tt5plus").html("<span><i class='ion-span ion-android-alert'></i>Please enter your FKEY</span>");
		} else {
			$("#i-tt5plus").html("<span><i class='ion-span ion-android-alert'></i>Please enter 4-11 vaild characters (English characters, numbers)</span>");
		}
		keycheck = false;
	} else {
...
function checkun(){
	...
	$.get(getUrl,function(str){
		if(str == '1'){
			...
			$("#i-tt1").html("<span><i class='ion-span ion-android-alert'></i>The username had already been registered!</span>");
		}
		$('#i-tt1').show();
	})
}
...
function checkem(){
	...
	$.get(getUrl,function(str){
		...
		if(str == '1'){
			...
			$("#i-tt2").html("<span><i class='ion-span ion-android-alert'></i>The email had already been registered!</span>");
		}
		$('#i-tt2').show();
	})
}
...
function ckboxOnClick(){
	if($("input[type='checkbox']").is(':checked')){
		...
		$("#i-tt7").html("<span style='top: 1px; left: 103px;'><i class='ion-span ion-android-alert'></i>Please check it</span>");
		$("#i-tt7").show();
	}
}
...
function btnclick(){
	if(check()){
		...
		// $("#i-tt6").html("<span style='top: 1px; left: 103px;'><i class='ion-span ion-android-alert'></i>Please finish the swipe</span>");
		// $("#i-tt6").show();
	// }
}
